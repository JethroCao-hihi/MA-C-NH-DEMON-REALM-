...
namespace UnityCopilot
{
    ...
    public static class UnityBridgeServer
    {
        ...
        private static void Start()
        {
            ...
            try
            {
                if (!TryStartOnAvailablePort(Port, 10))
                {
                    Debug.LogError($"[UnityCopilot] Failed to start bridge: no free port in range {Port}-{Port + 10}.");
                    return;
                }
        ...
        private static bool TryStartOnAvailablePort(int preferredPort, int maxOffset)
        {
            for (int offset = 0; offset <= maxOffset; offset++)
            {
                ...
                try
                {
                    ...
                    if (offset > 0)
                    {
                        Debug.LogWarning($"[UnityCopilot] Port {preferredPort} is busy. Using fallback port {candidatePort}.");
                    }
