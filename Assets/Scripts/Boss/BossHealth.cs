...
public class BossHealth : MonoBehaviour
{
    ...
    public void TakeDamage(float dmg)
    {
        ...
        if (bossController != null)
            bossController.OnBossDamaged();
    }

    public void InterruptHurtReaction()
    {
        if (anim == null)
            return;

        if (HasAnimatorTrigger(hurtTriggerName))
        ...
        if (useHurtVariants)
        {
            ...
            {
                string variant = $"{hurtTriggerName}_{i}";
                if (HasAnimatorTrigger(variant))
                    anim.ResetTrigger(variant);
            }
        }
    }
